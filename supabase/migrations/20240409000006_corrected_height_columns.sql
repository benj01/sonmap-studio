-- Add the correct height-related columns to geo_features table
ALTER TABLE geo_features
ADD COLUMN geometry_2d geometry(Geometry, 4326),
ADD COLUMN base_elevation_ellipsoidal double precision,
ADD COLUMN object_height double precision,
ADD COLUMN height_mode text,
ADD COLUMN height_source text,
ADD COLUMN vertical_datum_source text;

-- Create index on base_elevation_ellipsoidal for better query performance
CREATE INDEX idx_geo_features_base_elevation ON geo_features(base_elevation_ellipsoidal);

-- Add comments to the columns
COMMENT ON COLUMN geo_features.geometry_2d IS 'The WGS84 2D footprint of the feature';
COMMENT ON COLUMN geo_features.base_elevation_ellipsoidal IS 'The calculated WGS84 ellipsoidal height';
COMMENT ON COLUMN geo_features.object_height IS 'The height of the object itself (from attributes)';
COMMENT ON COLUMN geo_features.height_mode IS 'Metadata about how the height was determined (e.g., absolute_ellipsoidal)';
COMMENT ON COLUMN geo_features.height_source IS 'Metadata about the source of the height (e.g., attribute:H_MEAN, z_coord)';
COMMENT ON COLUMN geo_features.vertical_datum_source IS 'Metadata about the vertical datum (e.g., LHN95, WGS84)';

-- Update the transform_swiss_coords_swisstopo function to return correct field names
...
-- Update the import function to handle the new columns
...
DECLARE
  ...
  v_base_elevation_ellipsoidal FLOAT;
  v_object_height FLOAT;
  v_height_mode TEXT;
  ...
  FOR v_current_batch IN 0..v_batch_count-1 LOOP
    ...
    FOR i IN v_batch_start..v_batch_end-1 LOOP
      ...
      BEGIN
        ...
        -- Create 2D geometry
        v_geometry_2d := ST_Force2D(v_geometry);

        -- Extract height information from properties
        v_lhn95_height := (v_feature->'properties'->>'H_MEAN')::float;
        v_object_height := (v_feature->'properties'->>'HEIGHT')::float;
        
        -- Set metadata values
        v_height_mode := 'absolute_ellipsoidal';
        v_height_source := 'attribute:H_MEAN';
        v_vertical_datum_source := 'LHN95';
        ...
        -- Insert feature with all columns

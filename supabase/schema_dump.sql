...
CREATE FUNCTION public.check_file_import_status(file_id uuid) RETURNS json
...
CREATE FUNCTION public.check_function_details(function_name text) RETURNS TABLE(schema_name text, function_name text, argument_types text, return_type text, security_type text, is_strict boolean, description text)
...
CREATE FUNCTION public.debug_check_import(p_file_id uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE 
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'collections', (
      ...
      FROM geo_collections c
      WHERE c.project_id = pf.project_id AND c.file_id = pf.id
    )
  ) INTO v_result
...
CREATE FUNCTION public.delete_shapefile_companions() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.enable_rls_on_spatial_ref_sys() RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
...
CREATE FUNCTION public.force_mark_file_as_imported(file_id uuid, metadata jsonb) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.get_available_layers() RETURNS TABLE(layer_id uuid, layer_name text, feature_count bigint, bounds jsonb, properties jsonb)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
        ...
        SELECT 
            ...
            CASE 
                WHEN COUNT(gf.id) > 0 THEN
                    jsonb_build_object(
                        'bbox', ST_AsGeoJSON(ST_Extent(gf.geometry))::jsonb,
                        'center', jsonb_build_object(
                            'lng', ST_X(ST_Centroid(ST_Extent(gf.geometry))),
                            'lat', ST_Y(ST_Centroid(ST_Extent(gf.geometry)))
                        )
                    )
                ELSE NULL
            END as bounds,
...
CREATE FUNCTION public.get_imported_files(source_file_id uuid) RETURNS TABLE(id uuid, name text, file_type text, storage_path text, import_metadata jsonb, uploaded_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $_$
BEGIN
    RETURN QUERY
...
CREATE FUNCTION public.get_layer_features(layer_id uuid) RETURNS TABLE(id uuid, properties jsonb, geojson text, srid integer)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $_$
BEGIN
    RETURN QUERY
    SELECT 
        f.id,
        f.properties,
        ST_AsGeoJSON(ST_Transform(f.geometry, 4326)) as geojson,
        ST_SRID(f.geometry) as srid
    FROM geo_features f
    WHERE f.layer_id = $1;
END;
$_$;
...
CREATE FUNCTION public.get_layer_features_geojson(p_layer_id uuid) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_features jsonb;
        ...
        'features', COALESCE(
            jsonb_agg(
                jsonb_build_object(
                    'type', 'Feature',
                    'id', gf.id,
                    'geometry', ST_AsGeoJSON(ST_Transform(gf.geometry, 4326))::jsonb,
...
CREATE FUNCTION public.get_project_files_with_companions(project_id_param uuid) RETURNS TABLE(id uuid, name text, file_type text, storage_path text, size bigint, uploaded_at timestamp with time zone, is_shapefile_component boolean, companion_files jsonb)
    LANGUAGE plpgsql
    AS $$BEGIN
    RETURN QUERY
    SELECT 
...
CREATE FUNCTION public.get_project_member_counts(project_ids uuid[]) RETURNS TABLE(project_id uuid, count bigint)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
begin
...
CREATE FUNCTION public.get_shapefile_companions(main_file_id uuid) RETURNS TABLE(id uuid, name text, file_type text, storage_path text, component_type text, uploaded_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $_$
BEGIN
    RETURN QUERY
...
CREATE FUNCTION public.import_geo_features(p_project_file_id uuid, p_collection_name text, p_features jsonb, p_source_srid integer, p_target_srid integer) RETURNS TABLE(collection_id uuid, layer_id uuid, imported integer, failed integer)
    LANGUAGE plpgsql
    AS $$
DECLARE
  ...
  LOOP
    BEGIN
      ...
      BEGIN
        ...
        -- Finally transform
        v_geometry := ST_Transform(v_geometry, p_target_srid);
        
        RAISE NOTICE 'Geometry conversion successful: %', ST_AsText(v_geometry);
      EXCEPTION WHEN OTHERS THEN
        v_last_error := SQLERRM;
...
CREATE FUNCTION public.import_geo_features_test(p_project_id uuid, p_layer_name text, p_geometry json, p_properties jsonb, p_source_srid integer, p_target_srid integer) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
    ...
    INSERT INTO geo_features (
        ...
        v_layer_id,
        ST_Force3D(  -- This ensures the geometry is 3D
            ST_Transform(
                ST_SetSRID(
...
CREATE FUNCTION public.import_geo_features_with_transform(p_project_file_id uuid, p_collection_name text, p_features jsonb, p_source_srid integer, p_target_srid integer DEFAULT 4326, p_batch_size integer DEFAULT 100) RETURNS TABLE(collection_id uuid, layer_id uuid, imported_count integer, failed_count integer, debug_info jsonb)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.import_single_feature(p_layer_id uuid, p_geometry jsonb, p_properties jsonb, p_source_srid integer DEFAULT 2056) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_geometry GEOMETRY;
...
BEGIN
  ...
  BEGIN
    ...
    v_geometry := ST_GeomFromGeoJSON(v_cleaned_geom);
    RAISE NOTICE 'After ST_GeomFromGeoJSON: %', ST_AsText(v_geometry);
    
    v_geometry := ST_SetSRID(v_geometry, p_source_srid);
    RAISE NOTICE 'After ST_SetSRID: % (SRID: %)', ST_AsText(v_geometry), ST_SRID(v_geometry);
    
    v_geometry := ST_Transform(v_geometry, 4326);
    RAISE NOTICE 'After ST_Transform: % (SRID: %)', ST_AsText(v_geometry), ST_SRID(v_geometry);
  EXCEPTION WHEN OTHERS THEN
...
CREATE FUNCTION public.log_import(p_collection_id uuid, p_message text, p_level text DEFAULT 'info'::text, p_details jsonb DEFAULT NULL::jsonb) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
    INSERT INTO import_logs (level, message, details)
    VALUES (
        p_level,
        p_message,
        jsonb_build_object(
...
CREATE FUNCTION public.log_import_message(p_level text, p_message text, p_details jsonb DEFAULT NULL::jsonb) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
    INSERT INTO import_logs (level, message, details)
...
CREATE FUNCTION public.log_import_operation(p_operation text, p_details jsonb) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
    INSERT INTO import_logs(level, message, details)
...
CREATE FUNCTION public.monitor_imports() RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_start timestamp;
...
CREATE FUNCTION public.monitor_imports_v2() RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_count int;
...
CREATE FUNCTION public.monitor_imports_v3() RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_start_time timestamptz;
    ...
    WHILE clock_timestamp() - v_start_time < v_timeout LOOP
        ...
        WITH active_queries AS (
            ...
            WHERE query ~* 'import_geo_features_with_transform'
               OR query ~* 'INSERT INTO geo_features'
        )
...
CREATE FUNCTION public.set_uploaded_by() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.test_warnings() RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.track_import_progress(p_collection_id uuid, p_total_features integer, p_imported_count integer, p_failed_count integer, p_batch_number integer, p_total_batches integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
    INSERT INTO import_logs(level, message, details)
...
CREATE FUNCTION public.update_import_progress(p_import_log_id uuid, p_imported_count integer, p_failed_count integer, p_collection_id uuid DEFAULT NULL::uuid, p_layer_id uuid DEFAULT NULL::uuid, p_metadata jsonb DEFAULT NULL::jsonb) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
...
CREATE FUNCTION public.update_project_file_import_status(p_file_id uuid, p_is_imported boolean, p_import_metadata text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE FUNCTION public.update_project_storage() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE TABLE public.geo_features (
    ...
    geometry public.geometry(GeometryZ,4326) NOT NULL,
    properties jsonb DEFAULT '{}'::jsonb,
    srid integer NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT valid_geometry CHECK (public.st_isvalid(geometry))
);

...
BEGIN
  ...
  -- Also log to import_logs
  PERFORM log_import_message(
    'info',
    format('Starting import of %s features with SRID %s in %s batches', 
      v_total_features, p_source_srid, v_batch_count),
    jsonb_build_object(
  ...
  IF v_total_features > 0 THEN
    ...
    PERFORM log_import_message(
      'debug',
      'First feature structure received',
      jsonb_build_object('feature', p_features->0)
    );
  END IF;
  ...
  PERFORM log_import_message(
    'info',
    format('Created collection and layer. Target dimensions: %s', v_target_dims),
    jsonb_build_object(
  ...
  FOR v_current_batch IN 0..v_batch_count-1 LOOP
    ...
    PERFORM log_import_message(
      'info',
      format('Processing batch %s of %s', v_current_batch + 1, v_batch_count),
      jsonb_build_object(
    ...
    FOR i IN v_batch_start..v_batch_end-1 LOOP
      ...
      BEGIN
        ...
        IF v_feature->'geometry' IS NULL THEN
          ...
          PERFORM log_import_message(
            'warning',
            format('Feature %s has no geometry object', i + 1),
            jsonb_build_object('feature_index', i)
          );
          CONTINUE;
        END IF;
        ...
        IF v_raw_geometry IS NULL THEN
          ...
          PERFORM log_import_message(
            'error',
            format('Failed to parse geometry for feature %s', i + 1),
            jsonb_build_object(
              'feature_index', i,
        ...
        IF NOT ST_Equals(v_cleaned_geometry, v_raw_geometry) THEN
          ...
          PERFORM log_import_message(
            'info',
            format('Cleaned duplicate vertices for feature %s', i + 1),
            jsonb_build_object('feature_index', i)
          );
        END IF;
        ...
        IF NOT ST_IsValid(v_raw_geometry) THEN
          ...
          PERFORM log_import_message(
            'warning',
            format('Invalid geometry detected for feature %s', i + 1),
            jsonb_build_object(
              'feature_index', i,
          ...
          BEGIN
            ...
            IF NOT ST_IsValid(v_geometry) THEN
              ...
              PERFORM log_import_message(
                'info',
                format('Repaired geometry using ST_MakeValid for feature %s', i + 1),
                jsonb_build_object('feature_index', i)
              );
            ELSE
              ...
              PERFORM log_import_message(
                'info',
                format('Repaired geometry using ST_Buffer(0) for feature %s', i + 1),
                jsonb_build_object('feature_index', i)
              );
            END IF;
          ...
          EXCEPTION WHEN OTHERS THEN
            ...
            PERFORM log_import_message(
              'error',
              format('Failed to repair geometry for feature %s', i + 1),
              jsonb_build_object(
                'feature_index', i,
                'error', SQLERRM,
                'error_state', SQLSTATE,
        ...
        IF v_geometry IS NULL THEN
          ...
          PERFORM log_import_message(
            'error',
            format('Final geometry is NULL for feature %s', i + 1),
            jsonb_build_object('feature_index', i)
          );
          CONTINUE;
        END IF;
        ...
        IF ST_NDims(v_geometry) < v_target_dims THEN
          ...
          PERFORM log_import_message(
            'info',
            format('Added Z coordinate to geometry for feature %s', i + 1),
            jsonb_build_object('feature_index', i)
          );
        END IF;
        ...
        PERFORM log_import_message(
          'info',
          format('Successfully imported feature %s of %s', v_imported_count, v_total_features),
          jsonb_build_object(
            'feature_index', i,
            'total_features', v_total_features,
      ...
      EXCEPTION WHEN OTHERS THEN
        ...
        PERFORM log_import_message(
          'error',
          format('Failed to process feature %s', i + 1),
          jsonb_build_object(
            'feature_index', i,
            'error', v_last_error,
            'error_state', v_last_state

...
const SOURCE = 'api/height-transformation/feature-counts';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    logger.debug('Getting feature counts by height mode', { layerId });
    ...
    if (layerError) {
      logger.error('Failed to get layer', { error: layerError, layerId });
      return NextResponse.json(
    ...
    if (totalCountError) {
      logger.error('Failed to count features', { error: totalCountError, layerId });
      return NextResponse.json(
    ...
    if (lv95CountError) {
      logger.error('Failed to count LV95 features', { error: lv95CountError, layerId });
      return NextResponse.json(
    ...
    for (const mode of heightModes) {
      ...
      if (countError) {
        logger.warn('Failed to count features by height mode', { error: countError, heightMode: mode });
    ...
    if (queryError) {
      logger.warn('Failed to query features by height mode', { error: queryError });
    }
    ...
    logger.info('Feature counts retrieved successfully', { 
      layerId, 
      totalCount, 
      lv95Count
    });
    ...
    logger.error('Unexpected error getting feature counts', error);
    return NextResponse.json(
